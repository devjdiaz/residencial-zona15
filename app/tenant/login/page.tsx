...
export default function TenantLogin() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (authError) { setError("Credenciales incorrectas. Verifica con tu administradora."); return }
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Portal de inquilinos</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="tu-usuario@residencial.local"
            />
          </div>
          <div>
          ...
          <button type="submit" disabled={loading}
            ...
            style={{ background: "#b64532" }}>
            {loading ? "Entrando…" : "Entrar"}
          </button>
        </form>

        <p className="text-xs text-center text-gray-400 mt-4">
          ¿Olvidaste tus credenciales? Escríbele a tu administradora.
        </p>
      </div>
    </div>
  )
